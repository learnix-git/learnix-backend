...
export class UserController {
  static async update_info(req: Request, res: Response) {
    try {
  ...
  static async update_avatar(req: Request, res: Response) {
    try {
      const userId = res.locals.user.id;
  ...
  static async get_info(req: Request<{ id: string }>, res: Response) {
    try {
      const { id } = req.params;
  ...
  static async get_tutor_profile(req: Request<{ id: string }>, res: Response) {
    try {
      const { id } = req.params;
      const tutor = await UserService.get_tutor_profile(id);

      res.status(200).json({ code: 200, data: tutor });
    } catch (error: any) {
      res.status(400).json({
        code: 400,
  ...
  static async get_student_profile(req: Request<{ id: string }>, res: Response) {
    try {
      const { id } = req.params;
      const student = await UserService.get_student_profile(id);

      res.status(200).json({ code: 200, data: student });
    } catch (error: any) {
      res.status(400).json({
        code: 400,
  ...
  static async get_banks(req: Request, res: Response) {
    try {
      const userId = res.locals.user.id;
      const banks = await UserService.get_banks(userId);

      res.status(200).json({ code: 200, data: banks });
    } catch (error: any) {
      res.status(400).json({
        code: 400,
  ...
  static async create_bank(req: Request, res: Response) {
    try {
      const userId = res.locals.user.id;
      ...
      res.status(201).json({ code: 201, data: bank });
    } catch (error: any) {
      res.status(400).json({
        code: 400,
  ...
  static async delete_bank(req: Request<{ id: string }>, res: Response) {
    try {
      const userId = res.locals.user.id;
      const { id } = req.params;
      ...
      res.status(200).json({ code: 200, message: "Xoá ngân hàng thành công!" });
    } catch (error: any) {
      res.status(400).json({ 
        code: 400, 
